...
async fn project(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    let subcommand = match args.single::<String>() {
        ...
        Err(_) => {
            msg.reply(
                ctx,
                "❌ 사용법: `/project <generate | rename | delete> [프로젝트명]`",
            )
            .await?;
            return Ok(());
        }
    ...
    match subcommand.as_str() {
        ...
        "generate" => {
            ...
            if let Some(guild_id) = msg.guild_id {
                ...
                let exists = cache
                    .project_mapping
                    .keys()
                    .any(|k| k.contains(&project_name));
                if exists {
                    msg.reply(
                        ctx,
                        format!(
                            "❌ 이미 '{}' 이름의 프로젝트가 존재합니다. 다른 이름을 사용해주세요.",
                            project_name
                        ),
                    )
                    .await?;
                ...
                msg.reply(
                    ctx,
                    format!(
                        "🏗️ '{}' 프로젝트 생성을 시작합니다. 역할 및 채널을 세팅 중...",
                        project_name
                    ),
                )
                .await?;
                ...
                let bot_id = match ctx.http.get_current_user().await {
                    Ok(user) => user.id,
                    Err(why) => {
                        msg.reply(ctx, format!("❌ 봇 정보를 가져오지 못했습니다: {:?}", why))
                            .await?;
                        return Ok(());
                    }
                };
                ...
                let project_role = match guild_id.create_role(&ctx.http, role_builder).await {
                    Ok(role) => role,
                    Err(why) => {
                        msg.reply(ctx, format!("❌ 역할 생성 실패: {:?}", why))
                            .await?;
                        return Ok(());
                    }
                };
                ...
                if let Err(why) = ctx
                    .http
                    .add_member_role(guild_id, msg.author.id, project_role.id, None)
                    .await
                {
                    println!("⚠️ [generate] 유저에게 역할 부여 실패: {:?}", why);
                } else {
                    println!(
                        "✅ [generate] 유저에게 역할(ID: {}) 부여 완료",
                        project_role.id
                    );
                }
                ...
                match guild_id.create_channel(&ctx.http, category_builder).await {
                    ...
                    Err(why) => {
                        msg.reply(ctx, format!("❌ 카테고리 생성 실패: {:?}", why))
                            .await?;
                    }
                }
                // 캐시 리프래시
            ...
            if let Some(guild_id) = msg.guild_id {
                if let Channel::Guild(channel) = msg.channel_id.to_channel(&ctx.http).await? {
                    if let Some(category_id) = channel.parent_id {
                        ...
                        if let Ok(Channel::Guild(cat_channel)) =
                            category_id.to_channel(&ctx.http).await
                        {
                        ...
                        match category_id.edit(&ctx.http, builder).await {
                            ...
                            Err(why) => {
                                msg.reply(ctx, format!("❌ 이름 변경 실패: {:?}", why))
                                    .await?;
                            }
                        }
                    } else {
            ...
            if let Some(guild_id) = msg.guild_id {
                ...
                if let Channel::Guild(channel) = msg.channel_id.to_channel(&ctx.http).await? {
                    if let Some(category_id) = channel.parent_id {
                        ...
                        msg.reply(ctx, "🧹 프로젝트 채널들과 역할을 완전히 삭제합니다...")
                            .await?;
        ...
        _ => {
            msg.reply(
                ctx,
                "❌ 알 수 없는 하위 명령어입니다. (사용 가능: generate, rename, delete)",
            )
            .await?;
        }
    }
